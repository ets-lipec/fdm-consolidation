...
class Healing_Calculation:
    
    def __init__(self,solution,matrix_generation):
    ...
    def do_calculation(self,i,j):
        ...
        Vheal = np.zeros(len(self.solution.Ttot[0]))
        nxmid = min(i,j)*self.matrix_generation.nx
        ntini = min(i,j)*self.solution.nt
        
        for k in range (ntini,len(self.solution.Ttot[0])-1):
            
            tk = (k-ntini)*self.matrix_generation.dt
            tk1 = (k+1-ntini)*self.matrix_generation.dt
            Tav = ((self.solution.Ttot[nxmid,k+1]+self.solution.Ttot[nxmid,k])/2)
    ...
    def do_calculation3(self,i,j):
        ...
        for k in range (ntini,len(self.solution.Ttot[0])-1):
            ...
            Vheal[k+1] = min(1, (Vheal[k]**4+(1/(2*self.tw(self.solution.Ttot[nxmid,k+1]))+1/(2*self.tw(self.solution.Ttot[nxmid,k])))*self.matrix_generation.dt)**(1/4))
    ...
    def time_for_healing(self):
        ...
        for i in range (len(self.Thealtot)):
            self.Tforheal[i] = self.Thealtot[i]-(i+1)*self.solution.nt*self.matrix_generation.dt
