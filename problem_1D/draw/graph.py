...
class Graph:
    ...
    def do_temperature_graph(self):
        ...
        self.linestyles = np.array(["-","--","-.",":"])
        color = np.linspace(0,plt.cm.jet.N,self.matrix_generation.nlay,dtype=int)
        ...
        axes.plot(self.t,self.temperature.Ttot[0,:],"k-",label='Platform')
        for i in range (self.matrix_generation.nlay):
            for j in range (1,self.matrix_generation.nx+1):
                axes.plot(self.t[(i*self.temperature.nt):],self.temperature.Ttot[i*self.matrix_generation.nx+j,(i*self.temperature.nt):],self.linestyles[j-1],color=palette[i],linewidth=2,label='Layer '+str(i+1)+' Element '+str(j))
    ...
    def do_healing_graph(self):
        ...
        axes.set_title("Evolution of the healing during the time TRAPEZIUM METHOD (bed = 296K, time per layer = 100s)")
        axes.set_xlim((self.t.min(),self.t.max()))
